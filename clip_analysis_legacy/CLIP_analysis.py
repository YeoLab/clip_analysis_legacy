...
# from bin.clip_analysis_legacy import Robust_BAM_Reader
...
def small_peaks(feature):
    ...
    returns center of clip_analysis_legacy called peak (the middle of the narrow start / stop)
...
def count_genomic_region_sizes(regions, species="hg19", data_dir=DATA_DIR):
    ...
    for region in regions:
        region_tool = pybedtools.BedTool(os.path.join(data_dir, "regions", species + "_" + region + ".bed"))
...
def assign_to_regions(tool, clusters=None, assigned_dir=".", species="hg19", nrand=3, data_dir=DATA_DIR):
    ...
    for region in regions:
        bedtracks[region] = pybedtools.BedTool(os.path.join(data_dir, "regions", "%s_%s.bed" % (species, region)))
    #creates the basics of bed dict
    bed_dict = {'all': {'rand': {}}}

    genes = pybedtools.BedTool(os.path.join(data_dir, "regions", "%s_genes.bed" % (species)))
...
def get_offsets_bed12(bedtool):
    ...
    if bedtool.field_count() < 8:
        print("Not Valid bed12 file, making clipper_ish, and continuing processing")
        offset_dict = {}
        for interval in bedtool:
            interval = make_clipper_ish(interval)
            if interval.strand == "+":
...
def get_feature_distances(bedtool, features, exons):
    ...
    :param bedtool: bedtools of peaks, both  (only works with clip_analysis_legacy defined peaks)
...
def get_distributions(bedtool, region_dict):
    ...
    bedtool - clip_analysis_legacy bed file to generate distributions from
